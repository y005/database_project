...
setting_slave_db() {
  ##1. 마스터 데이터베이스 상태 확인
  master_log_file=`mysql -uroot -p'samho101' -h 192.168.0.3 -e "show master status\G" | grep mysql-bin`
  re="[a-z]*-bin.[0-9]*"
  if [[ ${master_log_file} =~ $re ]];then
      master_log_file=${BASH_REMATCH[0]}
  fi
  master_log_pos=`mysql -uroot -p'samho101' -h 192.168.0.3 -e "show master status\G" | grep Position`
  re="[0-9]+"
  if [[ ${master_log_pos} =~ $re ]];then
      master_log_pos=${BASH_REMATCH[0]}
  fi
  ##2. 마스터 데이터베이스 등록 후 슬레이브 구동
  query="change master to master_host='192.168.0.3', master_user='samho101', master_password='1234', master_log_file='${master_log_file}', master_log_pos=${master_log_pos}"
  mysql_note "${query}"
  mysql -uroot -p'samho101' -e "${query}"

  #3. 마스터 데이터베이스에 테이블을 생성한다. (마스터에서 복제 대상 테이블을 생성하기 전의 log 정보를 복제한다고 설정해야 한다.)
  mysql -uroot -p'samho101' -h 192.168.0.3 -e "create table if not exists database.test(id int auto_increment primary key,user_id varchar(50) null) engine=innodb;"
}
