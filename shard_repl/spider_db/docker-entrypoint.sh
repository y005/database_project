...
setting_sharding_db() {
	# 1. 샤딩에 필요한 스파이더 설치
  mysql -uroot -p'samho101' -e "source /usr/share/mysql/install_spider.sql"

  # 2. 2개의 데이터베이스 서버 설정
  mysql -uroot -p'samho101' -e "CREATE SERVER IF NOT EXISTS master_db1 FOREIGN DATA WRAPPER mysql OPTIONS(HOST '192.168.0.3',DATABASE 'database',USER 'user',PASSWORD '1234',PORT 3306);"
  mysql -uroot -p'samho101' -e "CREATE SERVER IF NOT EXISTS master_db2 FOREIGN DATA WRAPPER mysql OPTIONS(HOST '192.168.0.4',DATABASE 'database',USER 'user',PASSWORD '1234',PORT 3306);"
}
...
_main() {
	...
	if [ "$1" = 'mysqld' ] && ! _mysql_want_help "$@"; then
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
			...
			docker_setup_db
			mysql_note "Setting sharding db started."
			setting_sharding_db
			mysql_note "Setting sharding db finished."
